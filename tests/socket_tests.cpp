...
namespace
{
  ...
  void checkReceive(const error_code& ec, const std::optional<RadProto::Packet>& p, boost::asio::ip::udp::endpoint /*source*/)
  {
    ...
    std::set<uint8_t> types {
      RadProto::USER_NAME,
      RadProto::USER_PASSWORD,
      RadProto::NAS_IP_ADDRESS,
      RadProto::NAS_PORT,
      RadProto::MESSAGE_AUTHENTICATOR,
      RadProto::FRAMED_PROTOCOL};
...
BOOST_AUTO_TEST_CASE(TestConstructor)
{
  ...
  BOOST_CHECK_NO_THROW(RadProto::Socket s(
    io_service,
    "secret",
    3000,
...
BOOST_AUTO_TEST_CASE(TestAsyncSend)
{
  ...
  const std::vector<RadProto::Attribute*> attributes {
    new RadProto::String(1, "test"),
    new RadProto::Encrypted(2, "123456"),
    new RadProto::IpAddress(4, {127, 0, 0, 1}),
    new RadProto::Integer<uint32_t>(5, 1),
    new RadProto::Bytes(80, {0xf3, 0xe0, 0x00, 0xe7, 0x7d, 0xeb, 0x51, 0xeb, 0x81, 0x5d, 0x52, 0x37, 0x3d, 0x06, 0xb7, 0x1b}),
    new RadProto::Integer<uint32_t>(7, 1)};

  std::vector<RadProto::VendorSpecific> vendorSpecific;
  vendorSpecific.emplace_back(RadProto::VendorSpecific(171, 1, {0, 0, 0, 3}));
  ...
  RadProto::Socket s(io_service, "secret", 3000, checkReceive);
