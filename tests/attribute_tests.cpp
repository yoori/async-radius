#define BOOST_TEST_MODULE radius_liteattribute_tests
...
BOOST_AUTO_TEST_CASE(StringDataConstructor)
{
  ...
  radius_lite::String s(1, d.data(), d.size());
...
{
  radius_lite::String c(1, "test");
  std::unique_ptr<radius_lite::Attribute> cs(c.clone());
...
BOOST_AUTO_TEST_CASE(IntegerDataConstructor)
{
  ...
  radius_lite::Integer<uint32_t> s(5, d.data(), d.size());
...
BOOST_AUTO_TEST_CASE(IntegerDataConstructorThrow)
{
  ...
  BOOST_CHECK_THROW(radius_lite::Integer<uint32_t>(5, d.data(), d.size()), radius_lite::Exception);
}

BOOST_AUTO_TEST_CASE(IntegerValueConstructor)
{
  radius_lite::Integer<uint32_t> v(5, 169090600);
...
{
  radius_lite::Integer<uint32_t> c(5, 169090600);
  std::unique_ptr<radius_lite::Attribute> cs(c.clone());
...
BOOST_AUTO_TEST_CASE(IpAddressDataConstructor)
{
  ...
  radius_lite::IpAddress s(4, d.data(), d.size());
...
BOOST_AUTO_TEST_CASE(IpAddressDataConstructorThrow)
{
  ...
  BOOST_CHECK_THROW(radius_lite::IpAddress(4, d.data(), d.size()), radius_lite::Exception);
}

BOOST_AUTO_TEST_CASE(IpAddressValueConstructor)
{
  std::array<uint8_t, 4> address {127, 104, 22, 17};
  radius_lite::IpAddress v(4, address);
...
{
  std::array<uint8_t, 4> address {127, 104, 22, 17};
  radius_lite::IpAddress c(4, address);
  std::unique_ptr<radius_lite::Attribute> cs(c.clone());
...
BOOST_AUTO_TEST_CASE(EncryptedDataConstructor_PasswordLength_6)
{
  std::array<uint8_t, 16> auth {
    0x92, 0xfa, 0xa1, 0xed, 0x98, 0x9b, 0xb4, 0x79, 0xfe, 0x20, 0xe2, 0xf4, 0x7f, 0x4a, 0x5a, 0x70};
  std::vector<uint8_t> d {
    0x25, 0x38, 0x58, 0x18, 0xae, 0x97, 0xeb, 0xeb, 0xbd, 0x46, 0xfd, 0xb9, 0xd1, 0x17, 0x84, 0xeb};
  radius_lite::Encrypted s(2, d.data(), d.size(), "secret", auth);
...
BOOST_AUTO_TEST_CASE(EncryptedDataConstructorThrow)
{
  ...
  std::vector<uint8_t> d {
    0x25, 0x38, 0x58, 0x18, 0xae, 0x97, 0xeb, 0xeb, 0xbd, 0x46, 0xfd, 0xb9, 0xd1, 0x17, 0x84, 0xeb,
    0x23, 0x78, 0x44, 0x17, 0x2e, 0x27, 0xab, 0xbd, 0xeb, 0xa1, 0xb9, 0x17, 0xd2, 0x97, 0x87, 0x4d,
    0x35, 0x3d, 0x5a, 0xae, 0xa1, 0xc2, 0x4b, 0x5e, 0x2e, 0x5f, 0xfd, 0x9b, 0x18, 0x5a, 0x36, 0xe2,
    0x28, 0x48, 0x9b, 0x97, 0x6a, 0x5d, 0xed, 0x4d, 0x5d, 0x3d, 0x79, 0x4d, 0x5e, 0x7f, 0x3d, 0x5f,
    0x46, 0xed, 0x4f, 0x28, 0xbe, 0x98, 0x36, 0xb4, 0xa1, 0x44, 0x4b, 0xed, 0xb4, 0x17, 0x19, 0x6a,
    ...
    0x3b, 0x38, 0x5f, 0xeb, 0xe2, 0xd2, 0xa1, 0x5a, 0x1c, 0x97, 0x3d, 0xb9, 0x9b, 0xa1, 0x5f, 0xeb,
    0x2f
  };
  BOOST_CHECK_THROW(radius_lite::Encrypted(2, d.data(), d.size(), "secret", auth), radius_lite::Exception);
}

BOOST_AUTO_TEST_CASE(EncryptedDataConstructor1_PasswordLength_15)
{
  std::array<uint8_t, 16> auth {
    0x40, 0x66, 0x42, 0xfc, 0x3e, 0xa4, 0x30, 0x4a, 0x42, 0x39, 0xdb, 0xb1, 0xf9, 0x8a, 0x09, 0x40};
  std::vector<uint8_t> d {
    0xf9, 0xe4, 0x4a, 0x32, 0x8a, 0xee, 0x19, 0x48, 0x64, 0x50, 0x70, 0x31, 0xf0, 0x92, 0x06, 0x05};
  radius_lite::Encrypted s(2, d.data(), d.size(), "secret", auth);
...
{
  std::array<uint8_t, 16> auth {
    0x9c, 0x8e, 0xef, 0xce, 0x5f, 0x18, 0xc7, 0x30, 0x04, 0xc3, 0x34, 0xee, 0x80, 0xfd, 0xf3, 0x98};
  std::vector<uint8_t> d {
    0x6a, 0xd8, 0x72, 0x2f, 0x87, 0x8a, 0xd5, 0x79, 0x0c, 0x30, 0xc3, 0xf6, 0x41, 0x70, 0xd6, 0x82};
  radius_lite::Encrypted s(2, d.data(), d.size(), "secret", auth);
...
{
  std::array<uint8_t, 16> auth {
    0xf0, 0xcb, 0x8b, 0x5b, 0xd2, 0xd8, 0x96, 0x0b, 0xf7, 0x80, 0x68, 0x89, 0x27, 0x6a, 0xa4, 0xdc};
  std::vector<uint8_t> d {
    0x52, 0x8a, 0x79, 0x24, 0xf4, 0xa9, 0xc9, 0x04, 0x2b, 0x4a, 0xfe, 0x2f, 0x10, 0xd8, 0xa0, 0xcd,
    0x51, 0x99, 0xd3, 0xfd, 0xfb, 0xb0, 0xdc, 0x97, 0x6a, 0x19, 0xd6, 0xcc, 0x17, 0xfb, 0xff, 0x3b};
  radius_lite::Encrypted s(2, d.data(), d.size(), "secret", auth);
...
{
  radius_lite::Encrypted v(2, "123456");
  ...
  std::vector<uint8_t> expected {
    0x02, 0x12, 0x25, 0x38, 0x58, 0x18, 0xae, 0x97, 0xeb, 0xeb, 0xbd, 0x46, 0xfd, 0xb9, 0xd1, 0x17,
    0x84, 0xeb};
...
{
  radius_lite::Encrypted v(2, "123456789876543");
  ...
  std::vector<uint8_t> values = v.toVector("secret", auth);
  std::vector<uint8_t> expected {
    0x02, 0x12, 0xf9, 0xe4, 0x4a, 0x32, 0x8a, 0xee, 0x19, 0x48, 0x64, 0x50, 0x70, 0x31, 0xf0, 0x92,
    0x06, 0x05};
...
{
  radius_lite::Encrypted v(2, "1234567898765432");
  ...
  std::vector<uint8_t> values = v.toVector("secret", auth);
  std::vector<uint8_t> expected {
    0x02, 0x12, 0x6a, 0xd8, 0x72, 0x2f, 0x87, 0x8a, 0xd5, 0x79, 0x0c, 0x30, 0xc3, 0xf6, 0x41, 0x70,
    0xd6, 0x82};
...
{
  radius_lite::Encrypted v(2, "12345678987654321");
  ...
  std::vector<uint8_t> values = v.toVector("secret", auth);
  std::vector<uint8_t> expected {
    0x02, 0x22, 0x52, 0x8a, 0x79, 0x24, 0xf4, 0xa9, 0xc9, 0x04, 0x2b, 0x4a, 0xfe, 0x2f, 0x10, 0xd8,
    0xa0, 0xcd, 0x51, 0x99, 0xd3, 0xfd, 0xfb, 0xb0, 0xdc, 0x97, 0x6a, 0x19, 0xd6, 0xcc, 0x17, 0xfb,
    0xff, 0x3b};
...
{
  radius_lite::Encrypted c(2, "123456");
...
{
  radius_lite::Encrypted c(2, "123456789876543");
...
{
  radius_lite::Encrypted c(2, "1234567898765432");
...
{
  radius_lite::Encrypted c(2, "12345678987654321");
...
BOOST_AUTO_TEST_CASE(BytesDataConstructor)
{
  ...
  radius_lite::Bytes s(19, d.data(), d.size());
...
{
  radius_lite::Bytes v(19, {'1', '2', '3', 'a', 'b', 'c'});
...
{
  radius_lite::Bytes c(19, {'1', '2', '3', 'a', 'b', 'c'});
  std::unique_ptr<radius_lite::Attribute> cs(c.clone());
...
{
  std::vector<uint8_t> d {
    0x31, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66,
    0x67};
  radius_lite::ChapPassword s(3, d.data(), d.size());
...
BOOST_AUTO_TEST_CASE(ChapPasswordDataConstructorThrow)
{
  ...
  BOOST_CHECK_THROW(radius_lite::ChapPassword(3, d.data(), d.size()), radius_lite::Exception);
}

BOOST_AUTO_TEST_CASE(ChapPasswordValueConstructor)
{
  radius_lite::ChapPassword v(3, 0x31, {
    0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67});
  ...
  std::vector<uint8_t> values = v.toVector({}, {});
  std::vector<uint8_t> expected({
    0x03, 0x13, 0x31, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x61, 0x62, 0x63, 0x64,
    0x65, 0x66, 0x67});
...
{
  radius_lite::ChapPassword c(3, 0x31, {
    0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67});
  std::unique_ptr<radius_lite::Attribute> cs(c.clone());
...
BOOST_AUTO_TEST_CASE(VendorSpecificDataConstructor)
{
  ...
  radius_lite::VendorSpecific s(d.data());
...
BOOST_AUTO_TEST_CASE(VendorSpecificDataConstructorThrow)
{
  ...
  BOOST_CHECK_THROW(radius_lite::VendorSpecific(d.data()), radius_lite::Exception);
}

BOOST_AUTO_TEST_CASE(VendorSpecificValueConstructor)
{
  radius_lite::VendorSpecific v(171, 1, {0, 0, 0, 3});
